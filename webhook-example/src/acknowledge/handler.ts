...
import { AcknowledgeInput } from './input'
import { AcknowledgeOutput } from './output'
...
export const webhookCreateHandler =
    OperationHandlerSetup.configureTriggerCreateHandler<MyTriggerAuth, AcknowledgeInput>((handler) =>
        handler.usingComposite(async (auth, input, invoke) => OperationHandlerResult.success({})
        )
    );
export const webhookDestroyHandler =
    OperationHandlerSetup.configureTriggerDestroyHandler<MyTriggerAuth, AcknowledgeInput>((handler) =>
        handler.usingComposite(async (auth, input, invoke) => OperationHandlerResult.success({})
        )
    );

export const webhookRequestHandler =
    OperationHandlerSetup.configureTriggerRequestHandler<MyTriggerAuth, AcknowledgeInput, AcknowledgeOutput>((handler) =>
        handler.usingComposite(async (auth, { input, request }, invoke) => {
             ...
             *
             * It takes a value that has two fields, "input" of type "SimpleWebhookInput" (the input schema of this trigger) and "request" of type "TriggerOperationHttpRequest" which represents the http
             ...
             * 
             * The output also has two fields "output" of type "SimpleWebhookOutput" (the output schema of this trigger) and "response" of type "TriggerOperationHttpResponse" which represents the http
