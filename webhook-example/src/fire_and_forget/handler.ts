...
import { FireAndForgetInput } from './input'
import { FireAndForgetOutput } from './output'
...
export const webhookCreateHandler =
    OperationHandlerSetup.configureTriggerCreateHandler<MyTriggerAuth, FireAndForgetInput>((handler) =>
        handler.usingComposite(async (auth, input, invoke) => OperationHandlerResult.success({})
        )
    );

export const webhookDestroyHandler =
    OperationHandlerSetup.configureTriggerDestroyHandler<MyTriggerAuth, FireAndForgetInput>((handler) =>
        handler.usingComposite(async (auth, input, invoke) => OperationHandlerResult.success({})
        )
    );

export const webhookRequestHandler =
    OperationHandlerSetup.configureTriggerRequestHandler<MyTriggerAuth, FireAndForgetInput, FireAndForgetOutput>((handler) =>
