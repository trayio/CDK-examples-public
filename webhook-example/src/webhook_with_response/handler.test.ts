...
import { webhookCreateHandler, webhookDestroyHandler, webhookRequestHandler, webhookResponseHandler } from './handler'
import '@trayio/cdk-runtime/connector/operation/OperationHandlerTestRunner';

/*
 *  Because all of the trigger suboperations are separate handlers, we can test them using different test suites, one for each, just like with regular handlers
 */
...
OperationHandlerTestSetup.configureHandlerTest(
	...
	(handlerTest) =>
		handlerTest
			.usingHandlerContext('test')
			.nothingBeforeAll()
			.testCase('should create a response', (testCase) =>
				testCase
					.givenNothing()
					.when(() => 
						({ 
